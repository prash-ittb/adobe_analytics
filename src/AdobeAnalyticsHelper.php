...
class AdobeAnalyticsHelper {
  ...
  protected function renderCdnMarkup(array $build) {
    ...
    if ($cdnType == 'amazon') {
      ...
      $build['#footer_js_code'] = $this->cdnConfig->get($environment . '_footer_js_code');
      $build['#custom_tracking_js_before'] = $this->cdnConfig->get($environment . '_cdn_custom_tracking_js_before');
      $build['#custom_tracking_js_after'] = $this->cdnConfig->get($environment . '_cdn_custom_tracking_js_after');
    }
    else {
